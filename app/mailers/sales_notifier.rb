class SalesNotifier < ApplicationMailer
  ...
  def inform_admins(sale_id, payment)
    ...
    unless payment.present?
      ...
      mail(to: "#{admin_emails}", subject: "Inform Admin, Sale had been upload a payment!", from: "#{email_template.from}", boyd: "#{lot.name} unit had been upload a new payment, buyer name: #{@buyer.name}, Phone Number: #{@buyer.mobile_contact_number}")
    end
  end
