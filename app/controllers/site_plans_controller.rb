class SitePlansController < ApplicationController
  ...
  def create
    @site_plan = SitePlan.new(site_plan_params)

    respond_to do |format|
      if @site_plan.save
        format.html { redirect_to @site_plan, notice: 'Site plan was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @site_plan.update(site_plan_params)
        format.html { redirect_to @site_plan, notice: 'Site plan was successfully updated.' }
  ...
  def destroy
    @site_plan.destroy
    respond_to do |format|
      format.html { redirect_to site_plans_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def set_site_plan
      @site_plan = SitePlan.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def site_plan_params
      params.require(:site_plan).permit(:project_id)
    end
end
